...
def markBorders(sent,viewLabel,start,end):
...
class UITabularView():
    ...
    def addTokenDef(self, viewLabel = '', tokenType = '', tokenLabel = '', tokenIndex = 0):
        # print(">>>>>>>", viewLabel, tokenLabel, startToken, endToken)
        for sidx in range(len(self._sentences)):
            s = self._sentences[sidx]
            # print(">>>>>>>", "sentence", sidx, "tokens", len(s["tokens"]))
            if viewLabel in s["anns"]:
                ...
                # --
                # if start >=0 and end >= 0 and start < len(s["tokens"]) and end <= len(s["tokens"]):
                if token_index_rel >= 0 and token_index_rel < len(s["tokens"]):
                    # print(">>>>>>>", "adding")
                    # print(s["tokens"])
                    row = sannlab["lastRow"]
                    # if not row or row.lastTokenUsed > start:
    ...
    def addTokenView(self, view, viewLabel:str):
        ...
        for c in anns:
            ...
            self.addTokenDef(viewLabel = viewLabel, tokenType = tokenLabel, tokenLabel = tokenLabel, tokenIndex = tokenIndex )
        # print(">>" , "addtokenLabelView" , annLabel)
        
    def showView(self, viewName:str, viewLabel:str = None):
        ...
        if view.getType() == "AnnotationView.TokenView":
            self.addTokenView(view, viewLabel)
